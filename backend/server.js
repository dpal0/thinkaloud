...
// ─── Create HTTP server (needed for both Express and WebSocket) ───
const server = http.createServer(app)

// ─── WebSocket: /ws/stt — ElevenLabs real-time Speech-to-Text proxy ───
...
wss.on('connection', (clientWs) => {
  const apiKey = process.env.ELEVENLABS_API_KEY
  if (!apiKey) {
    clientWs.send(JSON.stringify({ message_type: 'server_error', detail: 'ELEVENLABS_API_KEY not set in .env' }))
    clientWs.close()
    return
  }

  const elWs = new WebSocket(ELEVEN_STT_URL, {
  ...
  // Browser → ElevenLabs: forward raw PCM as base64 chunks
  clientWs.on('message', (data) => {
    if (elWs.readyState !== WebSocket.OPEN) return
    const msg = JSON.stringify({
      message_type: 'input_audio_chunk',
      audio_base_64: Buffer.from(data).toString('base64'),
      sample_rate: 16000,
    })
    elWs.send(msg)
  })

  // ElevenLabs → Browser: forward transcript events
  elWs.on('message', (data) => {
    ...
    try {
      ...
      if (
        ['partial_transcript', 'committed_transcript', 'session_started'].includes(mt) ||
        mt.includes('error')
      ) {
  ...
  elWs.on('error', (err) => {
    try {
      clientWs.send(JSON.stringify({ message_type: 'server_error', detail: err.message }))
    } catch {}
  })
